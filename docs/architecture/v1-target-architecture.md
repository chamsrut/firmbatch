...
### 14.1 The protected staging subset: one origin, an identity broker, Cognito **[M3.3a]**

A repository decision after revision D.1, recorded in ADR 0011 and drawn in `docs/architecture/m3-3-aws-staging-topology.md`; it is not a source revision and it changes no invariant. The staging subset of the table above is: Route 53 and a regional ACM certificate; an internet-facing ALB reachable only from a structurally validated, human-reviewed reviewer CIDR allow-list, the same list on port 443 and on port 80, whose only action is the redirect to HTTPS, with exact-host matching and no WAF of its own; private ECS Fargate tasks from **one immutable image digest** — **two services**, the web/API service serving the compiled portal and `/v1/*` and an **identity-broker** service on `/auth/*`, and **three one-off tasks**, for migrations, database bootstrap and a manually invoked **identity binding** that holds only a dedicated binding database credential able to execute one binding function — each with its own task role, secret access and security group; private RDS PostgreSQL 16 (Single-AZ, encrypted, `rds.force_ssl`, seven-day PITR, deletion protection, no replica) accepting PostgreSQL only from those five security groups; one NAT gateway with a fixed egress IP; an S3 gateway endpoint; Secrets Manager containers whose database values are written by the bootstrap task rather than by Terraform; CloudWatch log groups with explicit retention and allow-listed metadata fields only; ALB access logs disabled. AWS-managed records that can hold identity data — CloudTrail, Cognito logging and export, SES records, the Cognito WAF's logs — are outside that allow-list and are inventoried, access-restricted and retention-limited at deployment.

**The customer application has one origin** (`https://staging.app.firmbatch.com` in staging): the browser reaches the API same-origin under `/v1/*` and never through a second host, because the `__Host-` cookie contract cannot span two. Milestone 3.3 establishes only that staging origin; production public hostnames are Milestone 8's, and the portal stays same-origin whatever they are. The compiled portal is served from the web/API image; CloudFront and static-site S3 hosting are deferred as a staging simplicity and security decision, not the production performance design.

**Cognito authenticates; Firmbatch authorizes.** One Cognito User Pool per environment, Managed Login at a custom domain under the customer origin's registrable domain (its certificate in `us-east-1`), a confidential app client with the authorization-code grant and PKCE only, invite-only users with password plus required TOTP, five-minute access and ID tokens and an eight-hour refresh and session window. The broker exchanges and validates tokens server-side, finds the identity's explicit `(issuer, subject)` binding — made only by the identity-binding task, never by email — and opens an ordinary Firmbatch session; every callback ends in a clean `303` redirect. **The browser receives no Cognito token**: only the opaque Firmbatch session credential, named `__Host-fb_session` with `SameSite=Strict`, the CSRF secret, and during login a short-lived `SameSite=Lax` transaction handle that is not a session. Logout goes through the broker and Cognito's logout endpoint. Not used: Identity Pools, ALB `authenticate-cognito`, Cognito groups for workspaces or roles, JWT authorization at the core API, social providers. PostgreSQL remains the only authority for authorization (invariant 1); the operator capacity agent is no part of this deployment (invariant 11); no GPU driver is enabled.

**Delivery** is Terraform in separate roots per environment (no workspaces), an S3 backend with the native lockfile in a KMS-encrypted, versioned state bucket, pinned versions, provider aliases for `eu-central-1` and `us-east-1`, no PostgreSQL provider or `local-exec`, no static AWS keys, and GitHub Actions through OIDC with two `main`-only, human-approved environments, distinct plan and apply roles and manually dispatched workflows: static checks on pull requests; a trusted plan in `staging-plan`, whose saved plan is a content-addressed object in a separate KMS-encrypted plan bucket under Object Lock governance retention and short lifecycle expiry — never a GitHub artifact — with only a sanitized summary shown; a human-approved apply in `staging-apply`, started with the reviewed object's version and expected SHA-256 and verified independently rather than on the plan job's own metadata; then application deployment by image digest with the migration task run first. Terraform state and saved plans are equally sensitive. Deployment is sequenced as four slices (M3.3a–d) of which only the last may create a resource, after a reviewed plan, a current cost estimate and explicit authorization.
...
**Repository amendments after D.1** (not source revisions; each recorded in its ADR and changing no invariant):

| Date | Section | Change |
| --- | --- | --- |
| 2026-09-13 | §14.1 (new) | Milestone 3.3a, ADR 0011: the protected staging subset of §14 — one customer origin with the API same-origin, two ECS services from one image digest including an identity broker, Cognito authenticating behind that broker with no token in the browser, RDS PostgreSQL 16 with the managed-RDS qualification, Terraform in separate roots with an S3 lockfile backend and OIDC-only delivery, deployment in four slices with only the last creating a resource. Corrected the same day after an independent review: saved plans as encrypted short-lived objects behind two protected GitHub environments; `__Host-fb_session` and a `Lax` OIDC handle; an identity-binding one-off task; the port 80 allow-list; clean callback redirects; AWS-managed identity records; production hostnames left to Milestone 8. Corrected again in a final pass: a dedicated identity-binding database boundary; `main`-only protected environments with distinct roles; plan approval independent of plan-produced metadata, in an Object-Locked plan bucket separate from state; structural reviewer allow-list validation; credential-safe browser evidence. §17 unchanged. |
